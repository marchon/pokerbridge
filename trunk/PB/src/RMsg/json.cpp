...
{
	//for(int i = 0/*obj->metaObject()->propertyOffset()*/ ;i<obj->metaObject()->propertyCount(); i++)
	//{
//		QMetaProperty &p = obj->metaObject()->property(i);
//		map[p.name()] = p.read(obj);
//	}
	int i;
	for( i=0;i<obj->metaObject()->propertyCount();i++)
	{
		QMetaProperty p = obj->metaObject()->property(i);
		map[p.name()] = obj->property(p.name());
	}
	QList<QByteArray> dynProps = obj->dynamicPropertyNames();
	for(i=0;i<dynProps.count();i++)
	{
		map[dynProps[i]]=obj->property(dynProps[i]);
	}
}
...
void ObjectVariant::variantToObject(const QVariantMap &map, QObject *obj)
{
	...
	{
		obj->setProperty(key.toAscii(), map[key]);
	}
...
QString ObjectVariant::toShortString(QObject *obj)
{
	QVariantMap map;
	objectToVariant(map, obj);
	return toShortString(map);
}

QString ObjectVariant::toShortString(const QVariantMap &map)
{
	QString result="(";
	bool first=true;
	Q_FOREACH(QString key, map.keys())
	{
		...
		if(map.value(key).type()==QVariant::Map)
			result.append(toShortString(map.value(key).toMap()));
		else
